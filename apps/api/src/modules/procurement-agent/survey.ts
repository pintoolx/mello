...
export function surveyCandidate(candidate: CandidateEvaluation, service: ServiceRecord,
  ...
  const matchesRequirements = service.active && !candidate.reasonCodes.includes("CATEGORY_MISMATCH") &&
  ...
  return {
    ...
    humanSummary: reasons.length ? `${sellerName} 尚未符合付款條件：${[...new Set(reasons)].join("、")}。`
      : `${sellerName} 符合本次需求，可由你確認選用。`,
  };
}
