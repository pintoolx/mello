...
- Credit：僅顯示查詢到的 USDC 餘額及 Mock Top up 按鈕。Mock 按鈕只顯示提示，不修改餘額或送出加值請求。
...
Credit 取自 `GET /api/v1/demo/health`。僅在 x402 模式、Base Sepolia 網路確認及買方錢包查詢成功時顯示餘額；模擬模式、查詢失敗或資料缺漏顯示 `— USDC`，不以假零代替。金額使用原子單位字串換算，保留 USDC 的六位精度。
