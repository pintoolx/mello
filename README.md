...
- **採購申請與服務探索**：描述需求、設定預算並可附上文件，選擇是否需要發票、是否需要 Mello Registry 認證；建立申請後自動比對服務，不需再按探索。
...
此指令會套用 migrations、在空資料庫建立 Demo 公司／政策與兩個 Demo Seller，並登錄個股分析、總經分析、加密市場資訊與期貨分析四項服務。既有公司與政策不會被初始化流程覆寫。四個服務共用兩個 Demo Seller，其中總經分析與加密市場資訊支援 Demo 發票，個股分析與期貨分析不支援；初始化不會自動核發 Mello Registry 認證。
...
登入後新增申請，例如需求說明填「我需要總經分析，關注亞洲市場利率與通膨」、預算填 `0.10 USDC`。本機首次操作請保留「需要發票」，取消「需要 Mello Registry 認證」，再依序操作「建立申請 → 等待服務比對 → 選擇總經分析 → 送出採購並開始付款」。預設會模擬第一次發票失敗，可在案件頁重試發票並查看對帳結果。若要驗證認證篩選，請依 [認證管理流程](docs/BAZAAR_IMPLEMENTATION.md)另行建立服務審查紀錄。
...
- **服務與資料範圍**：目前提供個股分析、總經分析、加密市場資訊與期貨分析四項 Demo 服務；報告由 Demo 程式產生，沒有串接真實市場資料。後續擴充服務類型、供應商與真實資料來源。
